...
class Action
{
	...
	function login()
	{
		...
		$q = $this->db->query("SELECT * FROM users WHERE username = '" . $this->db->real_escape_string($username) . "' ");
		if ($q && $q->num_rows > 0) {
			$user = $q->fetch_array();
			$hash = isset($user['password']) ? $user['password'] : '';
			if (password_verify($password, $hash) || $hash === $password) { // allow legacy plain if not yet migrated
				// Rehash legacy/plain MD5 on successful login
				if (!password_needs_rehash($hash, PASSWORD_DEFAULT)) {
					// If stored as plain or md5, update to strong hash
					if ($hash === $password || preg_match('/^[a-f0-9]{32}$/i', $hash)) {
	...
	{
		extract($_POST);
		$q = $this->db->query("SELECT * FROM users WHERE username = '" . $this->db->real_escape_string($email) . "' ");
		if ($q && $q->num_rows > 0) {
			$user = $q->fetch_array();
			$hash = isset($user['password']) ? $user['password'] : '';
			if (password_verify($password, $hash) || $hash === md5($password) || $hash === $password) {
				if (!password_needs_rehash($hash, PASSWORD_DEFAULT)) {
	...
	// Basic RBAC helper: check if current user type has permission key
	public static function hasPermission($perm)
	{
		// Map types to permissions (adjust as needed): 1=admin, 2=staff, 3=member
		$roles = [
			'1' => ['manage_users', 'manage_loans', 'manage_payments', 'view_reports', 'manage_settings'],
			'2' => ['manage_loans', 'manage_payments', 'view_reports'],
			'3' => ['view_self', 'apply_loan', 'view_payments']
		];
		$type = isset($_SESSION['login_type']) ? (string) $_SESSION['login_type'] : '';
		return isset($roles[$type]) && in_array($perm, $roles[$type], true);
	}
